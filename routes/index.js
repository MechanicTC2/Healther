...
router.get('/', async function(req, res, next) {
  res.render('index', {title: 'Login to Healthper'});
});

router.get('/login', function(req, res, next) {
  res.render('login', {title: 'Login to Healthper', info: 'Login with existing credentials:'});
});
router.get('/signup', function(req, res, next) {
  res.render('signup', {title: 'Signup for Healthper', info: 'Signup for Healthper, email and password:'});
});
router.post('/authenticateUser', async function(req, res, next) {
  const result = await login(req, res, next);
  console.log(result);
  if (result == "not enough cred")
    res.render('login', {title: 'Login to Healthper', info: 'Please fill in all fields. '});
  else if (result == "incorrect pass/email")
    res.render('login', {title: 'Login to Healthper', info: 'Incorrect password or email:'});
  else if (result == "user does not exist")
    res.render('login', {title: 'Login to Healthper', info: 'Email does not exist. If you do not have an account, please sign up:'});
  else if (result == "ETIMEDOUT")
    res.render('login', {title: 'Login to Healthper', info: 'Please check your connection and try again. If the issue persists, try reconnecting to the internet:'});
  else {
